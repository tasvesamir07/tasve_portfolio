        ...
        >
          ...
          <div className="flex items-center justify-between pb-3 border-b border-white/5">
            <div className="flex items-center gap-3">
              ...
              <span className="text-sm font-bold text-gray-400 font-mono">
                Experience #{idx + 1}:{' '}
                <span className="text-white">{e.title || 'Untitled Role'}</span>
              </span>
            </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div>
              ...
              <input
                value={e.company}
                onChange={(ev) => {
                  const u = { ...e, company: ev.target.value }
                  onUpdate(idx, u)
                }}
                className={inputClass}
                placeholder="e.g. Royal Bengal AI"
              />
            </div>
            <div>
              <label className="text-[10px] text-gray-500 font-bold uppercase tracking-wider block mb-1.5">
              ...
              <input
                ...
                className={inputClass}
                placeholder="e.g. October 2025 – July 2026"
              />
            </div>
          </div>
          ...
          <div>
            ...
            <textarea
              value={e.desc}
              onChange={(ev) => {
                const u = { ...e, desc: ev.target.value }
                onUpdate(idx, u)
              }}
              className={textareaClass}
              rows={4}
              placeholder="Describe the job responsibilities and achievements..."
            />
          </div>
        </div>
      ))}
