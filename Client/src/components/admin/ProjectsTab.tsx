        ...
        >
          ...
          <div className="flex items-center justify-between pb-3 border-b border-white/5">
            <div className="flex items-center gap-3">
              ...
              <span className="text-sm font-bold text-gray-400 font-mono">
                Project #{idx + 1}:{' '}
                <span className="text-white">{p.title || 'Untitled Project'}</span>
              </span>
            </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div>
              ...
              <input
                value={p.title}
                onChange={(e) => {
                  const u = { ...p, title: e.target.value }
                  onUpdate(idx, u)
                }}
                className={inputClass}
                placeholder="e.g. Certificate Studio"
              />
            </div>
            <div>
              <label className="text-[10px] text-gray-500 font-bold uppercase tracking-wider block mb-1.5">
              ...
              <input
                value={p.category}
                onChange={(e) => {
                  const u = { ...p, category: e.target.value }
                  onUpdate(idx, u)
                }}
                className={inputClass}
                placeholder="e.g. fullstack, ai, iot"
              />
            </div>
            <div>
              <label className="text-[10px] text-gray-500 font-bold uppercase tracking-wider block mb-1.5">
              ...
              <input
                value={p.tag}
                onChange={(e) => {
                  const u = { ...p, tag: e.target.value }
                  onUpdate(idx, u)
                }}
                className={inputClass}
                placeholder="e.g. Featured, AI Powered"
              />
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div>
              ...
              <input
                value={p.github}
                onChange={(e) => {
                  const u = { ...p, github: e.target.value }
                  onUpdate(idx, u)
                }}
                className={inputClass}
          ...
          <div className="bg-[#0b0d16]/80 border border-white/5 rounded-lg p-4 flex flex-col md:flex-row items-start md:items-center gap-4">
            ...
            <div className="flex-grow w-full">
              ...
              <input
                value={p.image}
                onChange={(e) => {
                  const u = { ...p, image: e.target.value }
                  onUpdate(idx, u)
                }}
                className={inputClass}
          ...
          <div className="flex items-center gap-3">
            ...
            <input
              value={p.diagram_url || ''}
              onChange={(e) => {
                const u = { ...p, diagram_url: e.target.value }
                onUpdate(idx, u)
              }}
              className={inputClass}
              placeholder="Architecture diagram URL (SVG/PNG)"
            />
          </div>
        </div>
      ))}
