...
import { Plus, Trash2, Save, Loader, ChevronUp, ChevronDown, GripVertical, Upload, ExternalLink } from 'lucide-react'

interface CertificationData {
  id?: number; title: string; issuer: string; date: string
  credential_url: string; image: string; sort_order: number
}
...
export default function CertificationsTab({ certifications, saving, onAdd, onUpdate, onDelete, onImageUpload, onMove, onSave }: Props) {
  return (
    <div className="flex flex-col gap-6">
      ...
      {certifications.length === 0 && <p className="text-gray-500 text-sm">No certifications yet.</p>}
      {certifications.map((c, idx) => (
        <div key={c.id || idx} className={cardClass + " flex flex-col gap-4 shadow-xl shadow-black/10"}>
          <div className="flex items-center justify-between pb-3 border-b border-white/5">
            <div className="flex items-center gap-3">
              ...
              <div className="flex gap-1 shrink-0">
                ...
                <button onClick={() => onMove(idx, 'down')} disabled={idx === certifications.length - 1} className={btnMove} title="Move Down">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              <label className="text-[10px] text-gray-500 font-bold uppercase tracking-wider block mb-1.5">Title</label>
              <input value={c.title} onChange={e => { const u = { ...c, title: e.target.value }; onUpdate(idx, u) }}
                className={inputClass} placeholder="e.g. AWS Certified Solutions Architect" />
            </div>
            <div>
              <label className="text-[10px] text-gray-500 font-bold uppercase tracking-wider block mb-1.5">Issuer</label>
              <input value={c.issuer} onChange={e => { const u = { ...c, issuer: e.target.value }; onUpdate(idx, u) }}
                className={inputClass} placeholder="e.g. Amazon Web Services" />
            </div>
            <div>
              <label className="text-[10px] text-gray-500 font-bold uppercase tracking-wider block mb-1.5">Date</label>
              <input value={c.date} onChange={e => { const u = { ...c, date: e.target.value }; onUpdate(idx, u) }}
                className={inputClass} placeholder="e.g. 2025" />
            </div>
            <div>
              <label className="text-[10px] text-gray-500 font-bold uppercase tracking-wider block mb-1.5">Credential URL</label>
              <input value={c.credential_url} onChange={e => { const u = { ...c, credential_url: e.target.value }; onUpdate(idx, u) }}
          ...
          <div className="bg-[#0b0d16]/80 border border-white/5 rounded-lg p-4 flex flex-col md:flex-row items-start md:items-center gap-4">
            <div className="w-full md:w-auto shrink-0">
              <label className="text-[10px] text-gray-500 font-bold uppercase tracking-wider block mb-1.5">Badge Image</label>
            ...
            <div className="flex-grow w-full">
              <input value={c.image} onChange={e => { const u = { ...c, image: e.target.value }; onUpdate(idx, u) }}
                className={inputClass} placeholder="Or paste badge image URL" />
            </div>
            {c.image && (
