...
import { Plus, Trash2, Save, Loader, ChevronUp, ChevronDown, GripVertical, Upload } from 'lucide-react'
...
export default function GalleryTab({ items, saving, onAdd, onUpdate, onDelete, onImageUpload, onMove, onSave }: Props) {
  return (
    <div className="flex flex-col gap-6">
      ...
      {items.map((item, idx) => (
        <div key={item.id || idx} className={cardClass + " flex flex-col gap-4 shadow-xl shadow-black/10"}>
          <div className="flex items-center justify-between pb-3 border-b border-white/5">
            <div className="flex items-center gap-3">
              ...
              <div className="flex gap-1 shrink-0">
                ...
                <button onClick={() => onMove(idx, 'down')} disabled={idx === items.length - 1} className={btnMove} title="Move Down">
            ...
            <button onClick={() => item.id && onDelete(item.id)} className={btnDanger} title="Delete">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              ...
              <input value={item.title} onChange={e => { const u = { ...item, title: e.target.value }; onUpdate(idx, u) }}
                className={inputClass} placeholder="e.g. Conference Talk 2025" />
            </div>
            <div>
              <label className="text-[10px] text-gray-500 font-bold uppercase tracking-wider block mb-1.5">Description</label>
              <input value={item.description} onChange={e => { const u = { ...item, description: e.target.value }; onUpdate(idx, u) }}
          ...
          <div className="bg-[#0b0d16]/80 border border-white/5 rounded-lg p-4 flex flex-col md:flex-row items-start md:items-center gap-4">
            <div className="w-full md:w-auto shrink-0">
              <label className="text-[10px] text-gray-500 font-bold uppercase tracking-wider block mb-1.5">Image</label>
            ...
            <div className="flex-grow w-full">
              <input value={item.image} onChange={e => { const u = { ...item, image: e.target.value }; onUpdate(idx, u) }}
