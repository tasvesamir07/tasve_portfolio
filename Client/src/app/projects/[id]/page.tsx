    ...
    <div className="min-h-screen bg-[#07090e]">
      ...
      <article className="max-w-4xl mx-auto px-6 pb-24">
        ...
        <div className="relative w-full h-64 md:h-80 rounded-xl overflow-hidden mb-8 bg-[#0f121d] border border-white/5">
          {project.image ? (
            <Image src={project.image} alt={project.title} fill className="object-cover" sizes="(max-width: 1200px) 100vw, 1200px" priority />
          ) : (
        ...
        <div className="flex flex-col md:flex-row md:items-start md:justify-between gap-4 mb-8">
          <div>
            <h1 className="font-heading font-extrabold text-3xl md:text-4xl text-white">{project.title}</h1>
        ...
        {project.tags.length > 0 && (
          <div className="mb-10">
            ...
            <div className="flex flex-wrap gap-2">
              {project.tags.map((t) => (
                <span key={t} className="font-mono text-xs bg-white/5 border border-white/5 text-gray-300 px-3 py-1.5 rounded-lg">
                  {t}
                </span>
              ))}
            </div>
