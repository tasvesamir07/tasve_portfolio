...
export async function GET() {
  try {
    ...
    const { data, error } = await supabaseAdmin.from('certifications').select('*').order('sort_order', { ascending: true })
...
export async function POST(req: Request) {
  try {
    ...
    const supabaseAdmin = getSupabaseAdmin()
    const { data, error } = await supabaseAdmin.from('certifications').insert(parsed.data).select().single()
    if (error) return NextResponse.json({ error: error.message }, { status: 500 })
    revalidateHome()
    return NextResponse.json(data)
  } catch (err) {
