...
export async function handleFileUpload(ctx: Context, oldUrl?: string): Promise<string> {
  const doc = ctx.message?.document
  if (!doc) throw new Error('No document found in message')
  const fileId = doc.file_id
  const file = await ctx.api.getFile(fileId)
  ...
  const { error } = await supabase.storage
    .from('Media')
    .upload(fileName, buffer, { contentType: doc.mime_type || `application/${ext}` })
  if (error) throw error
  
  const { data: { publicUrl } } = supabase.storage.from('Media').getPublicUrl(fileName)
  
  // Try to remove old file if oldUrl was provided
  if (oldUrl) {
    try {
      const match = oldUrl.match(/\/object\/public\/Media\/(.+)$/)
      if (match && match[1]) {
